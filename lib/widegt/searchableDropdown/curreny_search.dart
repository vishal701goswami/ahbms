...
class CurrenySearch extends StatefulWidget {
  final Function myfunction;
  final List<Curreny> items;
  final String dataID;
  const CurrenySearch(
      {super.key,
  ...
  @override
  State<CurrenySearch> createState() => _CurrenySearchState();
}

class _CurrenySearchState extends State<CurrenySearch> {
  bool drop = false;
  List<Curreny> _clientsList = [];
  String dropValue = "Select currency";
  @override
  void initState() {
    super.initState();
  ...
  logic() {
    for (int i = 0; i < _clientsList.length; i++) {
      if (widget.items[i].id == widget.dataID) {
        setState(() {
          dropValue = _clientsList[i].curreny;
        });
      }
    }
  }
  ...
  void myLogic(String enterKeyword) {
    ...
    final suggestions = _clientsList.where((element) {
      final statusValue = element.curreny.toLowerCase();
                                ...
                                child: InkWell(
                                  onTap: () {
                                    ...
                                    setState(() {
                                      drop = false;
                                      dropValue =
                                          "${_clientsList[index].symbol} ${_clientsList[index].curreny}";
                                    });
                                  },
                                  child: Padding(
                                    padding: const EdgeInsets.all(8.0),
